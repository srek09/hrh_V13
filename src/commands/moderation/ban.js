...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.setName('ban')
		.setDescription('Bans the member for specific time')
		...
		.addStringOption((option) =>
			option
				.setName('time')
				.setDescription('the time while the member is banned')
				.setRequired(true)
				.addChoice('1 day', '1d')
				.addChoice('3 days', '3d')
				.addChoice('7 days', '7d')
	...
	async execute(interaction, client) {
		...
		let reason
		const hasPermission = Banner.permissions.has('BAN_MEMBERS', true)
